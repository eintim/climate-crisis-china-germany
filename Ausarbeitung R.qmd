...
Für die Visualisierung wurde zusätzlich das Paket `ggstream` [@ggstream] verwendet. Um Kartendaten zu visualisieren, wurden die Pakete `giscoR` [@giscoR] und `sf` [@sf] verwendet. Anschließend wurden die Karten mit dem Paket `gganimate` [@gganimate] animiert. Damit die Animation gerendert werden kann kommen das Paket `gifski` [@gifski] und `transformr` [@transformr] zum Einsatz.
...
Beide Datensätze beinhalten gleiche Spalten, wie Bevölkerungsdaten oder Bruttoinlandsprodukte. Um eine Vergleichbarkeit beider Datensätze zu garantieren, werden beide Daten miteinander verglichen. Zwar stammen beide aus der selben Quelle [@OWID-Github], dennoch ist nicht garantiert, das die Daten übereinstimmen.
...
In beiden Datensätzen liegen die Daten exakt aufeinander (vgl. @fig-comparison_energy_and_co2_dataset, @fig-comparison_energy_and_co2_dataset_2). Der Energy-Datensatz reicht zwar nur bis 1900 zurück, was jedoch ausreichend ist. Beide Datensätze lassen sich direkt miteinander vergleichen. 
...
In der Spalte `cement_co2` sind sowohl Werte mit NA und auch 0. Da diese Werte in einer zeitlich logischen Reihenfolge vorkommen ist davon auszugehen das bei den NA Werten keine Messungen vorliegen. Deshalb werden die NA-Werte nicht weiter behandelt.
...
In diesem Plot (@fig-co2_per_year_per_captia) sind die Bevölkerungsgrößen mit einberechnet. Es zeigt sich, dass die Deutschland für die meiste Zeit deutlich mehr CO₂ ausgestoßen hat. Insbesondere das Wirtschaftswunder in Deutschland war stark auf Kohle und Öl aufgebaut (vgl. @fig-energy_distribution_germany). China und Deutschland haben 2021 ca. gleich viele CO₂ Emissionen pro Einwohner ausgestoßen. Trotzdem ist festzuhalten, dass in den letzten 20 Jahren die Tendenz in Deutschland sinkend war, während Chinas aggressive Wirtschaftspolitik immer mehr Emissionen in Kauf genommen hatte. Zwar kommen erneuerbare Energiequellen langsam, aber die fossilen Energieträger sind weiterhin die primären Energiequellen. Gerade Öl und Gas sind mit dem Abschalten vieler Atomkraftwerke [@AKW-Abschaltungen] noch wichtiger geworden. Jedoch ist das Ziel bis 2022 vollständig aus der Atomkraft auszusteigen nicht erreicht worden.
...
Chinas Energie kommt weiterhin zum größten Teil aus Kohle und Öl. Zwar sind auch hier Ansätze auf dem Weg zu erneuerbaren Energien, sie werden jedoch eher als zusätzliche Energiequelle verwendet statt fossile Energieträger abzulösen. Es wurde im Gegensatz zu Detschland aber deutlich stärker in Wasserkraft investiert, was aber auch geographische Hintergründe hat. Dadurch schadet China seinen Nachbarn wie Vietnam, da Flüsse immer weniger Wasser führen [@Lenz2020May].

Trotzdem ist festzuhalten, dass der Energieverbrauch pro Einwohner in Deutschland höher ist als in China. 1965 verbauchte Deutschland 18,4-mal mehr Energie pro Einwohner als China. In 2021 verbauchte Deutschland pro Einwohner ca. 1,37-mal mehr Energie. Zwar schließt sich diese Lücke, aber nicht indem in Deutschland weniger Strom verbaucht wird, sondern indem China aufschließt (vgl. @fig-energy_per_captia). Deutschlands Tendenz sinkt, liegt aber weiterhin über dem Stand von 1965.
...
In den Plots (@fig-co2_per_year_per_fossil_fuel-1, @fig-co2_per_year_per_fossil_fuel-2) werden die Quellen der CO₂ Emissionen dargestellt. Festzustellen ist, dass Kohle in Deutschland immer mehr an Bedeutung verliert. Öl und besonders Gas gewinnen an Bedeutung. Gerade Gas ist problematisch im Kontext, dass ein Großteil des Gases aus Russland stammte [@Bundesregierung-Gas].
...
In China verliert Kohle auch an Bedeutung, trotzdem stammen noch 70% der Emissionen aus Kohle. In Deutschland liegt der Anteil nur noch bei 35%. In China wird die Kohle jedoch nicht abgelöst, sondern es kommen zusätzliche Energiequellen hinzu (vgl. @fig-co2_per_year_per_fossil_fuel_china).
...
In dem Plot (@fig-co2_per_capita_europe) werden die CO₂ Emissionen pro Einwohner in Europa von 2000 bis 2021 dargestellt.
Über den Verlauf der Jahre ist in allen Ländern ein langsamer Abfall der Emissionen zu erkennen. Dennoch nimmt Deutschland im Vergleich zu anderen Ländern einen hohen Wert ein.
